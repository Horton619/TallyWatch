...
- **Fix: multiple beacons collapsing to one surface in Companion.** The stable
  ...
  fallback core v2 had, so it returns a zero-OUI address (`00:00:00:xx:xx:xx`)
  instead of the real MAC. Every beacon flashed with a v3 toolchain therefore
  persisted the same bogus id (e.g. `TallyWatch:000000xxxxxx`) and, because
  Companion routes Satellite surfaces by DEVICEID/SERIAL, only one beacon was
  ever visible at a time. A single beacon worked (nothing to collide with),
  which is why the first hardware bring-up didn't catch it. Now the id is read
  from eFuse via `esp_read_mac()`, which works with WiFi down; beacons that
  already stored a zero-OUI id (a real NIC never has one) regenerate a real one
  on the next boot — no factory reset needed. The USB `getabout` MAC field got
  the same fix.
